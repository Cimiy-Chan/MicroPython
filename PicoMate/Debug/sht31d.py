...
class SHT31(object):
    ...
    def get_temp_humi(self, resolution=R_HIGH, clock_stretch=True, celsius=True) -> tuple[float, float]:
        """
        ...
        if celsius:
            temp = round(-45.0 + (175.0 * (t / 65535)), 2)
        else:
            temp = round(-49.0 + (315.0 * (t / 65535)), 2)

        humi = round(100 * (h / 65535), 2)
        return temp, humi